      ...
      <nav className='hidden md:block'>
        <ul className='flex gap-5'>
          {links.map(({id, link, name}) => 
            <NavLink key={id} to={link}>{name}</NavLink>
          )}
        </ul>
      </nav>

      {/* Dropdown Nav menu */}
      <nav className={`absolute right-10 p-5 w-[250px] bg-white/20 backdrop-blur-lg border border-white/20 shadow-lg rounded-xl transition-all duration-500 ease-in-out ${btnClicked ? "translate-y-10 opacity-100" : "-translate-y-52 opacity-0"} md:hidden`}>
